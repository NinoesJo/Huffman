...
public class HuffProcessor {
	...
	public void compress(BitInputStream in, BitOutputStream out){
		...
		String eCode = encodings[PSEUDO_EOF];
		out.writeBits(eCode.length(), Integer.parseInt(eCode, 2));
		out.close();
	}

	private int[] getCounts(BitInputStream in) {
		int[] counts = new int[ALPH_SIZE];
		int currentBits = in.readBits(BITS_PER_WORD);
		
		while (currentBits != -1) {
			counts[currentBits]++;
			currentBits = in.readBits(BITS_PER_WORD);
		}
		return counts;
	}
	...
	private void makeEncodings(HuffNode root, String path, String[] encodings) {
		...
		if (root.left != null) makeEncodings(root.left, path + "0", encodings);
		if (root.right != null) makeEncodings(root.right, path + "1", encodings);
	}
